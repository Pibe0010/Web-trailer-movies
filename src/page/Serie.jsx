...
import { API_KEY_SERIES, API_URL_SERIES, IMAGEN_PATH_SERIES, URL_SERIES } from '../config';
...
// funcion para realizar la peticion get a la api
  const fetchMovies = async (searchKey) => {
    ...
    const {
      data: { results },
    } = await axios.get(`${API_URL_SERIES}/${type}/movie`, {
      params: {
        api_key: API_KEY_SERIES,
        query: searchKey,
      },
    });
    setMovies(results);
  ...
  const fetchMovie = async (id) => {
    const { data } = await axios.get(`${API_URL_SERIES}/movie/${id}`, {
      params: {
        api_key: API_KEY_SERIES,
        ...
        <main>
          {movie ? (
            <div
              ...
              style={{
                backgroundImage: `url("${IMAGEN_PATH_SERIES}${movie.backdrop_path}")`,
              }}
            >
              {playing ? (
                <>
        ...
        <div className=' grid grid-cols-1 gap-2 lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 relative justify-center '>
          {movies.map((movie) => (
            <div key={movie.id} className='' onClick={() => selectMovie(movie)}>
              <img src={`${URL_SERIES + movie.poster_path}`} alt='' className='cursor-pointer max-w-full h-96 min-w-full' />
